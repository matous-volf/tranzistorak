...
pub(crate) struct Bot {
    ...
    the state. */
    /// `None` if voice commands are disabled.
    #[allow(dead_code)]
    voice_command_transcriptor: Option<Arc<command::voice::Transcriptor<Arc<Self>>>>,
    #[allow(dead_code)]
...
impl Bot {
    pub(crate) async fn new(context: Context) -> Result<Arc<Self>, CreationError> {
        ...
        let voice_command_transcriptor = if voice::COMMANDS_ENABLED == "true" {
            Some(
                command::voice::Transcriptor::new(None)
                    .await
        ...
        new.command_executor
            ...
            .await;
        if let Some(voice_command_transcriptor) = new.voice_command_transcriptor.as_ref() {
            voice_command_transcriptor
...
impl player::VoiceTickCallback for Arc<Bot> {
    async fn on_voice_tick(&self, guild_id: GuildId, voice_tick: VoiceTick) {
        if let Some(voice_command_transcriptor) = self.voice_command_transcriptor.as_ref() {
            voice_command_transcriptor
